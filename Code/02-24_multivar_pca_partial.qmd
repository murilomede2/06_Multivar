...
- Based on analysis goal:  
  ...
  - Decrease number of dimensions  ###PCA
  - Create clusters  *k-means
  - Establish significant relationships among multiple predictors and an outcome.  *Randon forest

- Based on Y (outcome) existence:
    - Supervised  ###If you have a Y variable
    - Unsupervised  ###If you don't have a Y variable
...
### Important to see if there is a multicol
...
##Pull out the top lowest correlations
...
- Model estimates (slopes of the models) magnitude and direction (+ or -) can change for multicollinear variables compared to a non-multicollinear model.  
...
##Flat linear model means uncorrelated.
```
...
##Slope of the linear model shows how the 2 variables are highly correlated.
```
...
## The "tidy" function will show all the information about the linear model.
...
## Apr and Oct are very uncorrelated.
...
## Apr and May they are very correlated.
...
# Scree plot =  visualize how many dimensions are explaining the variance of the weather data.
...
```{r pca regression}
# Checking VIF =  see if the PCAs are uncorrelated or not, we want tha they are not.
